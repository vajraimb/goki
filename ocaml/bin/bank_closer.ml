(* GOKI bank closer — HKFRS 9 identities for banks.
   Same 16-d → 32 ReLU → 16 ReLU → 1 logit as the generic closer.
   Slots are pack-specific: N01 equity, B01 ECL rollforward,
   N03 cash+FX, N04 wholesale debt, N05 tax, B02 gross−ECL=net,
   B03 advances-to-deposits (analytic), B04 coverage (analytic).
   Label = 1 iff a bank identity was broken after notes were filled.
   PPE / GP / inventory are not in this vector. *)
open Base
open Ocannl
open Stdio
...
let () =
  ...
  printf "goki_bank_closer ready  features=%d  hid=%d/%d  (B01 ECL / B02 loans)\n"
