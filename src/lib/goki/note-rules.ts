...
export const NOTE_FIELDS: {
  ...
  { key: "tci", label: "综合收益总额", group: "n0" },
  { key: "buyback", label: "回购", group: "n0" },
  { key: "sbp", label: "股份支付", group: "n0" },
  { key: "nci", label: "少数股东", group: "n0" },
  { key: "otherEq", label: "其他权益", group: "n0" },
  { key: "ownerTx", label: "与所有者交易", group: "n0" },
  { key: "eqTransfer", label: "权益转拨", group: "n0" },
  { key: "ppeAdd", label: "购置", group: "n1" },
  { key: "ppeAcq", label: "合并取得", group: "n1" },
  { key: "ppeHfs", label: "转入待售", group: "n1" },
...
export function equityGap(prior: YearBooks, curr: YearBooks, n: NoteBooks): number {
  const tci = Math.abs(n.tci) >= 0.5 ? n.tci : curr.ni + n.oci;
  return curr.re - prior.re - (tci - curr.dividends - n.buyback + n.sbp + n.nci + n.otherEq + n.ownerTx + n.eqTransfer);
}
...
export function ppeGap(prior: YearBooks, curr: YearBooks, n: NoteBooks): number {
  return (
    curr.ppe -
    (prior.ppe + n.ppeAdd + n.ppeCip + n.ppeAcq - curr.da - n.ppeDisp - n.ppeHfs - n.ppeImpair + n.ppeFx + n.ppeReval)
  );
}
...
/** Dead: residual catch-alls are not written. Kept so callers compile. */
export function plugTruncation(_issuer: Issuer, notes: NoteBooks): NoteBooks {
  return notes;
}
